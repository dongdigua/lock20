...
fn schedule_lock(app: gtk::Application) {
    ...
    glib::timeout_add_seconds_local(1190, move || {
        let output = std::process::Command::new("notify-send")
            .args(["-A", "skip=skip", "-t", "10000", "-e",
                   "10 seconds remaining before lock",
                   "Your screen will get locked for 20 seconds to make sure that you relax your eyes."])
            .output()
            .expect("Failed to execute notify-send");

        debug!("{:?}", &output);
        if output.stdout.as_slice() == b"skip\n" {
